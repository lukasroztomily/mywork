    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if not user.is_blocked:
                ...
                messages.error(request, 'Tvuj ucet byl zablokovan')
...
def forgotpassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            send_email = EmailMessage(mail_subject, message, to=[to_email])
            send_email.send()
            messages.success(request, 'Nove heslo bylo odeslano na email')
            return redirect('login')
        else:
            messages.error(request, 'Shit')
...
def change_password (request):
    if request.method == 'POST':
        ...
        if password_new == password_confirm:
            ...
            success = user.check_password(password_current)
            if success:
                ...
                messages.error(request,'Err match')
                return redirect('change_password')               
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        user.save()
        messages.success(request, 'Muzes se prihlasit')
        return redirect('login')
    else:
        messages.error(request, 'Nemuzes se prihlasit')
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Reset hesla')
